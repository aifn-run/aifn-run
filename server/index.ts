...
async function saveFunction(req: IncomingMessage, res: ServerResponse, id?: string) {
  ...
  if (typeof input.providerKey === 'string' && input.providerKey)
    await auth.setProperty(requestOf(req), `aifn:provider:function:${functionId}:key`, input.providerKey);
...
async function api(req: IncomingMessage, res: ServerResponse, url: URL) {
  ...
  if (parts[1] === 'provider' && parts[2] === 'account') {
    ...
    if (req.method === 'GET') {
      const endpoint = await auth.property(requestOf(req), 'aifn:provider:account:endpoint');
      const model = await auth.property(requestOf(req), 'aifn:provider:account:model');
      return send(res, 200, {
        endpoint: endpoint?.value || '',
        model: model?.value || '',
        keyConfigured: Boolean((await auth.property(requestOf(req), 'aifn:provider:account:key'))?.value),
      });
    }
    if (req.method === 'PUT') {
      let input: any;
      ...
      try {
        if (input.endpoint !== undefined)
          await auth.setProperty(requestOf(req), 'aifn:provider:account:endpoint', String(input.endpoint));
        if (input.model !== undefined)
          await auth.setProperty(requestOf(req), 'aifn:provider:account:model', String(input.model));
        if (input.key !== undefined && input.key !== '')
          await auth.setProperty(requestOf(req), 'aifn:provider:account:key', String(input.key));
        return send(res, 200, {
          endpoint: input.endpoint || '',
          model: input.model || '',
          keyConfigured: Boolean(input.key),
        });
      } catch (cause) {
        return error(
          res,
  ...
  if (parts[1] === 'fn' && parts.length === 2 && req.method === 'GET' && url.searchParams.get('visibility') === 'all') {
    ...
    return send(res, 200, rows.map((row) => ({ functionId: row.function_id, version: Number(row.version), name: row.name, model: row.model, provider: row.provider_slug || (row.provider_endpoint ? new URL(row.provider_endpoint).hostname : 'default'), output: row.output, public: Boolean(row.is_public) })));
  }
