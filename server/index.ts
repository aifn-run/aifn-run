...
type FunctionVersion = { functionId: string; version: number; prompt: string; name: string; model: string; format: string; output: OutputMode; providerEndpoint: string; active?: boolean };
...
async function migrate() {
  ...
  try { await database.run(`ALTER TABLE function_versions ADD COLUMN provider_endpoint TEXT NOT NULL DEFAULT ''`); } catch {}
}
...
function requestOf(req: IncomingMessage) { return new Request(`http://${req.headers.host || "localhost"}${req.url || "/"}`, { headers: { cookie: req.headers.cookie || "" } }); }
async function profileId(req: IncomingMessage) { return (await auth.session(requestOf(req)))?.id || null; }
function versionJSON(row: any): FunctionVersion { return { functionId: row.function_id, version: Number(row.version), prompt: row.prompt, name: row.name, model: row.model, format: row.format, output: row.output, providerEndpoint: row.provider_endpoint || "", active: Boolean(row.active) }; }
...
async function saveFunction(req: IncomingMessage, res: ServerResponse, id?: string) {
  ...
  await database.run(`INSERT INTO function_versions (function_id, version, prompt, name, model, format, output, provider_endpoint, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [functionId, version, input.prompt, input.name || "", input.model || "", input.format || "chat", input.output || "json", input.providerEndpoint || "", now]);
  if (typeof input.providerKey === "string" && input.providerKey) await auth.setProperty(requestOf(req), `aifn:provider:function:${functionId}:key`, input.providerKey);
...
async function complete(fn: FunctionVersion, input: unknown, req: IncomingMessage) {
  ...
  const response = await fetch(endpoint, { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${key}` }, body: JSON.stringify({ model: fn.model || process.env.API_MODEL, messages, response_format: json ? { type: "json_object" } : undefined }) });
...
async function api(req: IncomingMessage, res: ServerResponse, url: URL) {
  ...
  if (parts[1] === "provider" && parts[2] === "account") { if (!(await auth.session(requestOf(req)))) return error(res, 401, "AUTHENTICATION_REQUIRED", "Authentication required"); if (req.method === "GET") { const endpoint = await auth.property(requestOf(req), "aifn:provider:account:endpoint"); const model = await auth.property(requestOf(req), "aifn:provider:account:model"); return send(res, 200, { endpoint: endpoint?.value || "", model: model?.value || "", keyConfigured: Boolean((await auth.property(requestOf(req), "aifn:provider:account:key"))?.value) }); } if (req.method === "PUT") { let input: any; try { input = JSON.parse(await body(req)); } catch { return error(res, 400, "INVALID_JSON", "Request body must be JSON"); } try { if (input.endpoint !== undefined) await auth.setProperty(requestOf(req), "aifn:provider:account:endpoint", String(input.endpoint)); if (input.model !== undefined) await auth.setProperty(requestOf(req), "aifn:provider:account:model", String(input.model)); if (input.key !== undefined && input.key !== "") await auth.setProperty(requestOf(req), "aifn:provider:account:key", String(input.key)); return send(res, 200, { endpoint: input.endpoint || "", model: input.model || "", keyConfigured: Boolean(input.key) }); } catch (cause) { return error(res, 502, "AUTH_PROPERTY_ERROR", cause instanceof Error ? cause.message : "Unable to save provider settings"); } } }
